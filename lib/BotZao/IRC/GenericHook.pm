...
sub init(@args) {
	...
	$bot->hook(
		...
		sub {
			...
			log_debug('irc: generic hook: event: ' . $m->{command});
			log_debug('irc: generic hook: event generated by ' . $in->{nick});

			foreach my $plugin (@generic_plugins) {
				# Check for matching generic plugin trigger text
				next unless ($m->{command} =~ /$plugin->{trigger}/);
				log_debug("irc: $plugin->{name}: triggered");
				...
				# server-side actions.
				log_info("irc: $plugin->{name}: no messages to be sent back");
				return;
			}
			log_info('irc: generic hook: no plugin triggered');
		},
	);
	return;
}
