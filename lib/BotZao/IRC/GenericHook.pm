	...
	$bot->hook(
		...
		sub {
			...
			foreach my $plugin (@generic_plugins) {
				...
				# server-side actions.
				log_info("$plugin->{name}: no messages to be sent back");
				return;
			}
			log_info('no plugin triggered');
		},
	);
	return;
}
...
sub load($plugins) {
	log_debug('load ' . $_->{name}) foreach @$plugins;
